...
export default function Dashboard({ user, onLogout }) {
  ...
  async function loadClassroomData() {
    ...
    if (cr.teacher_id) {
      ...
      if (tnotes) {
        const map = {}
        tnotes.forEach(n => { map[Number(n.vehicle_id)] = n.notes || [] })
        setTeacherNotes(map)
      }
    }
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          {joinError && (
            <div style={{ background: dark ? '#2a0a0a' : '#fde8e8', border: `1px solid ${dark ? '#6b2200' : '#d93025'}`, borderRadius: 8, padding: '10px 14px', color: dark ? '#f87171' : '#b91c1c', fontSize: 13, marginBottom: 14, textAlign: 'left' }}>
              {joinError}
            </div>
          )}
          <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())}
            placeholder="z.B. AB12CD" autoFocus
          ...
          <button onClick={joinClassroom} disabled={joinLoading || !joinCode.trim()} style={{ width: '100%', padding: '13px 0', background: '#3b82f6', border: 'none', borderRadius: 10, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.08em', opacity: joinLoading ? 0.6 : 1, marginBottom: 16 }}>
  ...
  if (hwMode && homework) {
    ...
    const hwVehicles = ALL_VEHICLES.filter(v => homework.vehicle_ids?.includes(String(v.id)))
    ...
    return (
      <div style={{ minHeight: '100vh', background: bg, fontFamily: 'Arial', fontSize, color: text }}>
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '20px 14px 80px' }}>
          <div style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 12, padding: '20px 20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ fontSize: fontSize - 2, color: dim, marginTop: 4 }}>{hwVehicles.length} Fahrzeuge zugewiesen</div>
              </div>
              <button onClick={importHomework} style={{ padding: '10px 20px', background: '#22c55e', border: 'none', borderRadius: 10, color: '#fff', fontSize: fontSize - 1, fontWeight: 700, cursor: 'pointer', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
                ▶ IMPORTIEREN
              </button>
            </div>
            <div style={{ marginTop: 14, padding: '10px 14px', background: dark ? '#0d2a1a' : '#f0fdf4', border: `1px solid ${dark ? '#1e5f3e' : '#86efac'}`, borderRadius: 8, fontSize: fontSize - 3, color: dark ? '#4ade80' : '#166534' }}>
              💡 Mit <strong>IMPORTIEREN</strong> werden nur diese Fahrzeuge in der Auswahl aktiviert.
            </div>
          </div>
          <div style={{ fontSize: fontSize - 3, color: dim, letterSpacing: '0.1em', marginBottom: 10 }}>FAHRZEUGE IN DIESER HAUSAUFGABE</div>
          {hwVehicles.length === 0 ? (
            <div style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 10, padding: '24px', textAlign: 'center', color: dim, fontSize: fontSize - 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {hwVehicles.map((v, i) => {
                ...
                return (
                  <div key={v.id} style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 8, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: catInfo ? catInfo.color + '30' : bord, border: `1px solid ${catInfo ? catInfo.color + '60' : bord}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: fontSize - 4, fontWeight: 700, color: catInfo ? catInfo.light : dim, flexShrink: 0 }}>
                      {i + 1}
                    </div>
                    <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, fontFamily: 'Arial', fontSize, color: text, position: 'relative' }}>
      ...
      {showSidebar && (
        <div onClick={() => setShowSidebar(false)} style={{ position: 'fixed', inset: 0, background: '#00000080', zIndex: 40 }} />
      )}
      ...
      <div style={{ position: 'fixed', top: 0, left: showSidebar ? 0 : '-320px', bottom: 0, width: 300, background: surf, borderRight: `1px solid ${bord}`, zIndex: 50, transition: 'left 0.2s ease', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '10px 16px', borderBottom: `1px solid ${bord}`, display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => { const base = getFilteredVehicles(activeCat, superCat); setSelectedIds(selectedIds ? null : base.map(v => v.id)) }} style={{ ...btn(false, bord, dim), flex: 1, fontSize: fontSize - 3 }}>AUFHEBEN</button>
        </div>
      ...
      <div style={{ background: surf, borderBottom: `1px solid ${bord}`, position: 'sticky', top: 0, zIndex: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
            ...
            {classroom && (
              <button onClick={loadClassroomData} disabled={refreshing} title="Hausaufgaben und Lehrer-Merkmale aktualisieren" style={{ ...btn(false, bord, dim), padding: '4px 9px', fontSize: 14 }}>
                {refreshing ? '⏳' : '🔄'}
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'flex', overflowX: 'auto', borderTop: `1px solid ${bord}` }}>
          ...
          {classroom && !homework && (
            <span style={{ flex: '0 0 auto', padding: '8px 10px', color: dim, fontSize: fontSize - 4 }}>🏫 {classroom.name}</span>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '12px 14px 100px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button style={{ ...btn(mode === 'quiz', tc, tl), opacity: !poolHasImages ? 0.45 : 1 }} onClick={() => { if (poolHasImages) { setMode('quiz'); setChosen(null); setRevealed(false) } }} title={!poolHasImages ? 'Bilder noch nicht verfügbar' : ''}>◉ BILD-QUIZ</button>
        ...
        {/* CARD */}
        <div className="fade-in" key={cur.id} style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 12, overflow: 'hidden', marginBottom: 14 }}>
          ...
          <div style={{ background: tc + '18', borderBottom: `1px solid ${tc + '40'}`, padding: '10px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: fontSize - 4, color: dim, letterSpacing: '0.14em' }}>{cat.label.toUpperCase()} · {cat.sub}</div>
              <div style={{ fontSize: fontSize + 2, fontWeight: 700, color: text, letterSpacing: '0.05em', marginTop: 2 }}>
                {mode === 'quiz' && chosen === null ? '???' : cur.name}
                {(mode !== 'quiz' || chosen !== null) && <span style={{ fontSize: fontSize - 1, marginLeft: 8, opacity: 0.7 }}>{cur.flag}</span>}
              </div>
                  ...
                  {cur.images.length > 1 && (
                    <>
                      <button onClick={() => setImgIdx(i => ((i - 1 + cur.images.length) % cur.images.length))}
                        style={{ position: 'absolute', left: 6, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.45)', border: 'none', borderRadius: '50%', width: 32, height: 32, color: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>‹</button>
                      <button onClick={() => setImgIdx(i => (i + 1) % cur.images.length)}
                        style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.45)', border: 'none', borderRadius: '50%', width: 32, height: 32, color: '#fff', fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 }}>›</button>
                    </>
                  )}
                </div>
                {cur.images.length > 1 && (
                  <div style={{ display: 'flex', gap: 6, padding: 8 }}>
                    {cur.images.map((_, i) => (
                      <button key={i} onClick={() => setImgIdx(i)} style={{ width: 8, height: 8, borderRadius: '50%', border: 'none', background: i === imgIdx % cur.images.length ? tl : dim, cursor: 'pointer', padding: 0 }} />
                    ))}
                  </div>
                )}
              </>
            ...
            <div style={{ padding: '14px 16px', borderTop: `1px solid ${bord}` }}>
              {!poolHasImages ? (
                <div style={{ background: dark ? '#12181f' : '#f0f4f8', border: `1px solid ${bord}`, borderRadius: 8, padding: '14px 16px', textAlign: 'center', fontSize: fontSize - 2, color: dim }}>
                  Das Bild-Quiz ist verfügbar sobald Fahrzeugbilder geladen sind.
                  <br />
                  <a href={getRecomonkeyUrl(cur, activeCat)} target="_blank" rel="noopener noreferrer" style={{ color: tl, marginTop: 6, display: 'inline-block' }}>
                ...
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {opts.map(o => {
                      ...
                      return (
                        <button key={o.id} onClick={() => pick(o.id)} style={{ background: bg2, border: `1px solid ${border2}`, borderRadius: 8, padding: '10px 12px', color: col2, fontSize: fontSize - 2, fontWeight: showResult && isCorrect ? 700 : 400, textAlign: 'left', cursor: chosen !== null ? 'default' : 'pointer', transition: 'all 0.15s', fontFamily: 'Arial' }}>
                          {o.flag} {o.name}
                        </button>
                      )
                    })}
                  ...
                  {chosen !== null && (
                    <div style={{ marginTop: 12, fontSize: fontSize - 2, fontWeight: 700, color: chosen === cur.id ? ansCorrect.col : ansWrong.col }}>
      ...
      {showJoin && (
        <div style={{ position: 'fixed', inset: 0, background: '#00000090', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowJoin(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: surf, border: `1px solid ${bord}`, borderRadius: 14, padding: '28px 28px', width: '100%', maxWidth: 360, fontFamily: 'Arial' }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: text, marginBottom: 6 }}>🏫 Klassenraum beitreten</div>
            <div style={{ fontSize: 12, color: dim, marginBottom: 18 }}>Gib den Code ein, den du von deinem Lehrer erhalten hast.</div>
            {joinError && (
              <div style={{ background: dark ? '#2a0a0a' : '#fde8e8', border: `1px solid ${dark ? '#6b2200' : '#d93025'}`, borderRadius: 8, padding: '9px 14px', color: dark ? '#f87171' : '#b91c1c', fontSize: 13, marginBottom: 12 }}>{joinError}</div>
            )}
